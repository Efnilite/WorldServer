...
public class WorldServerMenu {
    ...
    public static void openMainMenu(Player player) {
        ...
        if (player.hasPermission("ws.spy")) {
            menu.item(9, new SliderItem().initial(wp.isSpyMode() ? 0 : 1).add(0, new Item(Material.LIME_STAINED_GLASS_PANE, "<#0DCB07><bold>Spy mode enabled").lore("<gray>See what every player", "<gray>in every world is saying."), (event) -> {
                wp.setSpyMode(true);
                return true;
        ...
        if (player.hasPermission("ws.reload")) {
            menu.item(13, new Item(Material.HOPPER, "<blue><bold>Reload files").lore("<gray>This will reload all files.").click((event) -> {
                ...
                }), event, WorldServer.getPlugin()).stay(20 * 5));
